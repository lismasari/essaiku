...
# KONFIGURASI
# ══════════════════════════════════════════
# API key dan password TIDAK ditulis langsung di sini.
# Keduanya diambil dari file .streamlit/secrets.toml (lokal) atau
# menu "Secrets" di Streamlit Community Cloud (saat deploy online).
#
# Isi .streamlit/secrets.toml harus seperti ini:
#   GROQ_API_KEY = "gsk_xxxxxxxxxxxxxxxxxxxx"
#   DOSEN_PASSWORD = "password_dosen_kamu"
#
# Jangan lupa tambahkan ".streamlit/secrets.toml" ke file .gitignore
# agar tidak ikut terunggah ke GitHub atau repositori publik lainnya.
try:
    GROQ_API_KEY   = st.secrets["GROQ_API_KEY"]
    DOSEN_PASSWORD = st.secrets["DOSEN_PASSWORD"]
except (KeyError, FileNotFoundError):
    st.error(
        "⚠️ Secrets belum diatur. Buat file .streamlit/secrets.toml berisi "
        "GROQ_API_KEY dan DOSEN_PASSWORD terlebih dahulu, lalu jalankan ulang aplikasi."
    )
    st.stop()
...
def page_login():
    ...
    with col:
        ...
        # Catatan: password TIDAK ditampilkan di halaman ini.
        # Password hanya diberikan langsung (secara pribadi) oleh admin/pembuat
        # sistem kepada dosen yang berhak, di luar aplikasi ini.
        st.markdown("""
        ...
        pwd  = st.text_input("Password", type="password", placeholder="Masukkan password...", key="inp_pwd")
...
def page_dosen():
    ...
    if h == "Dashboard":
        # Deteksi alamat website otomatis agar link mahasiswa akurat
        try:
